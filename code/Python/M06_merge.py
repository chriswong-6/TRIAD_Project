...
#print(df_2012Ethiopia_merge)

#copy booleanize table
for coun in Country:
    for year in country_year[coun]:
        names['df_'+str(year)+str(coun)+'_final'] = (names['df_'+str(year)+str(coun)+'_merge'])[['Bmean','Bmedian','Bmax','Bmin']].copy()


# change input data in each rows to numpy
CounYear_input_np = df_2001Egypt_final.values
#change output data in each rows to numpy
CounYear_output_np = df_2001Egypt_merge['Year'].values
