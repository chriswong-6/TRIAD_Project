...
for year in range(1999,2014):
    for coun in Country:
        names['df_curr'+str(year)+str(coun)] = df_test2.loc[(df_test2['Year'] == year)&(df_test2['Country'] == coun)].copy()
        names['df_others'+str(year)+str(coun)] = df_test2.loc[(df_test2['Year'] != year) | (df_test2['Country'] != coun)].copy()
...
for b in range(len(year_self_pd)):
    ...
    if year_self_pd[b].iloc[0,2] >= year_others_pd[b].nums.mean():
        year_self_pd[b].iloc[0,4] = 1
    else:
        year_self_pd[b].iloc[0,4] = 0

for b in range(len(year_self_pd)):
    #for i in range(year_self_pd[b]):
    if year_self_pd[b].iloc[0,2] >= year_others_pd[b].nums.median():
        year_self_pd[b].iloc[0,6] = 1
    else:
        year_self_pd[b].iloc[0,6] = 0

for b in range(len(year_self_pd)):
    #for i in range(year_self_pd[b]):
    if year_self_pd[b].iloc[0,2] >= year_others_pd[b].nums.max():
        year_self_pd[b].iloc[0,8] = 1
    else:
        year_self_pd[b].iloc[0,8] = 0

for b in range(len(year_self_pd)):
    #for i in range(year_self_pd[b]):
    if year_self_pd[b].iloc[0,2] <= year_others_pd[b].nums.min():
        year_self_pd[b].iloc[0,10] = 1
    else:
        year_self_pd[b].iloc[0,10] = 0
...
CounYear_input_np = df_2000Ethiopia_final.values
#change output data in each rows to numpy
CounYear_output_np = df_2000Ethiopia_merge['Year'].values
